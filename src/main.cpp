...
std::vector<dvdJpeg> jpegs;
std::vector<std::string> dirs;
std::vector<std::string> menufiles;
std::vector<infoFile> infofiles;
...
void done()
{
    ...
    consoleColorRestore();
#endif
    exit (_xcode);  // in vista this causes std::endless recursion via atexit()
}
...
uint16_t init (int argc, char *argv[])
{
    ...
    if (! fs::exists (lplexConfig))
        {
            ...
            configFile <<  "formatout = wav"  << std::endl;
            configFile <<   "video = ntsc" << std::endl;
            configFile << "md5aware = yes"  << std::endl;
            configFile << "restore = yes"  << std::endl;
            configFile << "infofiles = yes"  << std::endl;
            configFile << "jpeg  = black"  << std::endl;
            configFile << "splice = seamless"  << std::endl;
            configFile << "shift = backward"  << std::endl;
            configFile << "cleanup = yes"  << std::endl;
#ifdef lgzip_support
            configFile << "create = lgz"  << std::endl;
#else
            configFile << "create = iso"  << std::endl;
#endif
            configFile << "media = dvd+r"  << std::endl;
            configFile << "dvdpath = adjacent"  << std::endl;
            configFile << "isopath = adjacent"  << std::endl;
            configFile << "extractpath = adjacent"  << std::endl;
            configFile << "workpath = " << job.tempPath.string()  << std::endl;
            configFile << "readonlypath = " << readOnlyPath.string() << std::endl;
            configFile <<  "verbose = no"  << std::endl;
            configFile.flush();
        }
    ...
    // ...or read in config file settings
        ...
        for (optindl = 0; long_opts[optindl].name; ++optindl)
            {
...
uint16_t addFiles (fs::path filespec)
{
    ...
    if (fs::exists (filespec))
        {
            ...
            std::string comp = filespec.filename().string();
    ...
    //if authoring, check if project
        ...
        {
            std::string comp = toUpper (Right (filespec.string(), 6));
...
int checkName (std::string &jobName, bool trim)
{
    ...
    if (Right (jobName,  9) == "_UNPACKED")
        {
            ...
            if (Right (jobName, 13) == "_DVD_UNPACKED")
                ...
                if (Right (jobName, 15).Left (5) == "_DVD_")
                    {
                        suffix = 15;
...
//    defaultName :
// ----------------------------------------------------------------------------
//    Returns a std::string of the form YYYY-MM-DD_HHMM
// ----------------------------------------------------------------------------
...
std::string defaultName()
{
    ...
    std::stringstream st;
    st << std::put_time (localtime (&now), "%Y-%m-%d_%H%M");
    return st.str();
}
...
bool isSubStr (std::string &A, std::string &B)
{
...
void splitPaths()
{
    ...
    for (uint i = 0; i < Lfiles.size(); ++i)
        {
            std::string dir = Lfiles[i].fName.string();
...
std::string  lFileTraverser::setRoot (const char *rootPath, int fromParent)
{
...
void lFileTraverser::OnFile (const std::string& filename)
{
    filenames.push_back (filename);
}


void lFileTraverser::Traverse (const std::string &path)
{
            ...
            if ( (lFile.format = isLfile (lFile.fName.extension().string().c_str()))
                    && (lFile.format == wavef || lFile.format == flacf))
                {
                    ...
                    if (! ok)
                        ...
                        if (lpcmEntity::soundCheck (&lFile, false))
                            {
                                if (startNewTitleset)
                                    {
                                        lFile.group = ++job.group;
                ...
                if (job.params & info && Right (filename, 9) != "lplex.log")
                    {
                        ...
                        if (job.params & unauth)
                            {
                                if ( (iFile.fName.substr (root, 8) == "VIDEO_TS" ||
                                    ...
                                    if ( (iFile.fName.substr (root, 4) == "XTRA")
...
int lFileTraverser::OnDir (const std::string& dirname)
{
    if (! dirSpecified ||
...
void lFileTraverser::OnOpenError (const std::string& openerrorname)
{
    FATAL ("Can't open '" + openerrorname + std::string ("'."));
}
...
void getOpts (const char *filename)
{
    ...
    if (! stdArgs (argc, argv, args, size))
        {
            delete args;
            FATAL ("Open-ended quotation in " + std::string (filename));
        }
...
bool getOpts (int argc, char *argv[])
{
    ...
    int _optind = optind;            // (index into parent argvstd::vector)
...
uint16_t setopt (uint16_t opt, const char *optarg)
{
    ...
    if (optarg == nullptr || ! stricmp (optarg, "yes") || ! stricmp (optarg, "true") || ! stricmp (optarg, "1"))
        ...
        if (! stricmp (optarg, "no") || ! stricmp (optarg, "false") || ! stricmp (optarg, "0"))
            isFalse = true;
    ...
    switch (opt)
        {
            ...
            case 'f':
                if (! strncmp (optarg, "flac", 4))
                    ...
                    if (! stricmp (optarg, "wave"))
                        job.format = wavef;

                    else
                        if (! stricmp (optarg, "wav"))
                            job.format = wavef;

                        else
                            if (! stricmp (optarg, "raw"))
                                job.format = lpcmf;

                            //#ifdef lgzip_support
                            else
                                if (! stricmp (optarg, "lgz"))
                                    job.format = lgzf;
            ...
            case 'l':
                if (! stricmp (optarg, "none"))
                    ...
                    if (! stricmp (optarg, "pad"))
                        job.trim = jobs::discrete;

                    else
                        if (! stricmp (optarg, "discrete"))
                            job.trim = jobs::discrete;

                        else
                            if (! stricmp (optarg, "padded") || ! stricmp (optarg, "indiscrete"))
                                {
                                    job.trim = jobs::discrete | jobs::padded;
                                }

                            else
                                if (! stricmp (optarg, "seamless"))
            ...
            case 's':
                ...
                if (! stricmp (optarg, "backward"))
                    ...
                    if (! stricmp (optarg, "nearest"))
                        job.trim |= jobs::nearest;

                    else
                        if (! stricmp (optarg, "forward"))
            ...
            case 'c':
                ...
                if (! stricmp (optarg, "lpcm"))
                    ...
                    if (! stricmp (optarg, "m2v"))
                        job.prepare = m2vf;

                    else
                        if (! stricmp (optarg, "mpeg"))
                            job.prepare = mpegf;

                        else
                            if (! stricmp (optarg, "dvdStyler"))
                                {
                                ...
                                if (! stricmp (optarg, "dvd"))
                                    job.prepare = vobf;

                                else
                                    if (! stricmp (optarg, "iso"))
                                        job.prepare = isof;
            ...
            case 'L':
                if (! stricmp (optarg, "none") || isFalse)
                    ...
                    if (! stricmp (optarg, "bright") || isTrue)
                        setcolors (bright);

                    else
                        if (! stricmp (optarg, "dark"))
                            setcolors (dark);

                        else
                            if (! stricmp (optarg, "ansi"))
                                colorMode = ansi;
    ...
    if (! (edit & piped))
        {
            ...
            if (! optFile.is_open())
                FATAL ("Can't open project file '" + projectFile + std::string ("'"));
        }
            ...
            if (! (edit & piped))
                {
                    ...
                    INFO ("Writing project file '"
                          + projectFile + std::string ("'\n\n"));
                }
        }

    constexpr const char *T = "yes", *F = "no";
    int bitPos[] = { 0, 1, 2, 0, 3, 0, 0, 0, 4 };
    dotLplex << shebang << std::endl <<
             "# lplex version " << LPLEX_VERSION_STRING  << std::endl <<
             "# Project : " << job.name << std::endl <<
             "# " << layout->spaceTxt << std::endl << std::endl <<
             "# Settings:\n\n";

    if (job.params & redirect && editing == absolute)
        dotLplex <<
                 "--dir="         << QUOTE ( (job.outPath.parent_path() /  job.name).string()) << std::endl; // -d

    else
        dotLplex <<
                 "--name="        << QUOTE (job.name) << std::endl;  // -n

    dotLplex <<
             "--md5aware="    << (job.params & md5 ? T : F) << std::endl << // -m
             "--infoFiles="   << (job.params & info ? T : F) << std::endl << // -x
             "--splice="      << (
                 job.trim0 & jobs::discrete ? job.trim0 & jobs::padded ? "padded" : "discrete" :
                 job.trim0 & jobs::seamless ? "seamless " : "none") << std::endl <<  // -l
             "--shift="       << (
                 job.trim0 & jobs::backward ? "backward" :
                 job.trim0 & jobs::nearest ? "nearest" :
                 job.trim0 & jobs::forward ? "forward" : "") << std::endl <<  // -s
             "--create="      << (job.params & dvdStyler ? "dvdStyler" :
                                  ( (const char*[])
    { "lpcm", "m2v", "mpeg", "dvd", "iso", "lgz"
    }) [ job.prepare - 3 ]) << std::endl << // -c
                            "--media="       << ( (const char*[])
    { "dvd+r", "dvd-r", "dl", "none"
    }) [ bitPos[ job.media ] ] << std::endl <<  // -z
                               "--cleanup="     << (job.params & cleanup ? T : F) << std::endl; // -C

    if (jpegs.size() == 1)
        dotLplex <<
                 "--jpeg="        << (alias (jpegs[0].fName) ?
                                      jpegs[0].fName.string() : QUOTE (jpegs[0].fName.string())) << std::endl;   // -j
    ...
    if (editing == absolute)
        {
            ...
            //     "--extractPath=" << QUOTE( job.extractPath.parent_path() ) << std::endl << // -e
        }

    dotLplex <<
             "--verbose="     << (_verbose ? T : F) << std::endl << // -v
             (generating ? "#" : "") <<
             "--editing="     << ( (const char*[])
    { "false", "relative", "absolute"
    }) [ editing ]
            << (edit & editVerbose ? ",v" : "") << std::endl;   // -Y
                    ...
                    if (lFile->group != titleset)
                        {
                            ...
                            dotLplex << (titleset ? "\nts" : "") <<
                                     _f ("\n# Title %d - (%s / %s %s)\n",
                                         ...
                                         jpegs[lFile->jpgIndex].sizeStr(), jpegs[lFile->jpgIndex].aspStr()) << std::endl;
                        }
                    ...
                    if (lFile->trim.type != trimType && ! (lFile->trim.type & jobs::autoSet))
                        {
                            ...
                            dotLplex << (
                                         ...
                                         trimType & jobs::seamless ? "seamless " : "none") << std::endl;
                        }
