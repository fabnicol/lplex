...
// ----------------------------------------------------------------------------
//    Returns std::string describing audio characteristics of given descriptor <l>.
// ----------------------------------------------------------------------------


std::string lpcmEntity::audioInfo (lpcmEntity *l)
{
...
std::string lpcmEntity::audioInfo (FLAC__StreamMetadata *fmeta)
{
    return _f ("%d bit %d khz %d ch",
        ...
        switch (ch)
            {
                ...
                case  4:
#endif
                    if (ch <= maxDvdChans[ (bps - 16) / 4][ (khz / 48) - 1])
                        break;
    ...
    if (! bpsOk || ! khzOk || ! chOk)
        {
            if (! mute)
                {
                    ERR ( (bpsOk ? std::string ("") : _f ("%d bit ", bps))
                          + (khzOk ? std::string ("") : _f ("%d khz ", khz))
                          + (chOk  ? std::string ("") : _f ("%d ch ",  ch))
                          + std::string ("lpcm is not ") + (notAllowed ? "allowed in dvd-video\n" : "supported\n"));
                }
...
//       <prefix> - std::string to prefix to stderr messages.
//
...
bool lpcmProcessor::md5compare (const void *md5, const char * prefix)
{
    ...
    ECHO (" : " << hexStr (md5, 16) << " : " << fName.stem() << std::endl);
...
//    lpcmWriter::swap2wav :
// ----------------------------------------------------------------------------
//    Swaps given lpcm data buffer from dvd to wave_bytes-order.
//
//    Arguments:
//       <data>            - pointer to start of wave data buffer
//       <count>           - number of_bytess to process
...
//       <size>   - number of_bytess to process
//
//    Returns 0 on success
// ----------------------------------------------------------------------------
...
uint16_t rawWriter::md5Report()
{
    ...
    LOG ("-dvd order md5" << (ct.max ? "    : " : " : ")
         << hexStr (md5strRaw, 16) << " : (raw)" << std::endl);
    return true;
}
...
//       <size>   - number of_bytess to process (must be aligned to one
//                  interchannel sample)
//
//    Returns 0 on success
// ----------------------------------------------------------------------------
...
uint32_t flacWriter::process (unsigned char *uBuf, uint32_t size)
{
    ...
    if (fmeta.data.stream_info.bits_per_sample == 16)
        ...
        if (fmeta.data.stream_info.bits_per_sample == 24)
            {
...
// ----------------------------------------------------------------------------
//    Swaps and sorts given lpcm data buffer from dvd_bytes-order into separate
...
//       <dvdCount>        - number of dvd_bytess to process
//       <channels>        - number of audio channels
//       <bitspersample>   - audio quantization
//
//    Returns 0 on success
...
#define LO(b)   (b & 0x0f)
            ...
            if (bitspersample == 24)
            ...
            else
                {
                    ...
                    ERR (std::to_string (bitspersample) + "bit audio is not supported\n");
                }
