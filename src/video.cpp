...
class gopHeader
{
    public:
        ...
        static void display (const gopInfo *GOP, const char* prefix = "", std::ostream &stream = std::cerr);
        friend std::ostream& operator << (std::ostream& stream, const gopInfo &g)
        {
            display (&g, "", stream);
            return stream;
        }
...
//       <sizeofUData>  - number of_bytess of userData
//       <framesPerGOP> - ...
                    ...
                    bool append, bool endSeq, bool close)
{
    ...
    (binDir / "mpeg2enc").string(),
    {
        ...
        "-G",
        std::to_string (framesPerGOP).c_str(),
        "-g",
        std::to_string (framesPerGOP).c_str(),
        "-o",
    ...
    std::ifstream miniFile ( (m2vPath / "mini.m2v").string(), std::ios::binary | std::ios::ate);

    if (! miniFile.is_open())
        FATAL ("Can't find input file " + (m2vPath / "mini.m2v").string());

    static std::ofstream m2vFile;
            ...
            if (bigBlock[look] == 0)
                {
                    if (bEndian32 (bigBlock + look) == 0x000001B3)
                        ...
                        if (bEndian32 (bigBlock + look) == 0x000001B8)
                            {
                                GOP[seqCt] = look + 4;
    ...
    //   gopHeader::display( &GOPheader, LOG_TAG,std::cerr );
    fps = tv == NTSC ? 30 : 25;
    fpm = fps * 60;
    fph = fps * 3600;
    ...
    if (close)
        {
            m2vFile.seekp (0, std::ios::end);
            size_t filesize = m2vFile.tellp();
            INFO ("Closing \'" <<   _m2vName << "\': " << filesize << "_bytess\n");
            m2vFile.close();
        }
...
void gopHeader::display (const gopInfo *GOP, const char* prefix, std::ostream &stream)
{
    stream << prefix <<
           "dropFrame  :" << GOP->dropFrame << std::endl << prefix <<
           "timestamp  :" << GOP->hour << ":" << GOP->min << ":" << GOP->sec << "." << GOP->frame << std::endl << prefix <<
           "closed     :" << GOP->closed << std::endl << prefix <<
           "broken     :" << GOP->broken << std::endl;
}
...
//    of average gop_bytess and adds it to the jpegFile array.
//
//    Arguments:
...
{
    std::string name = jpeg.stem().string();
    ...
    if (name == "black" || name == "black_XS" || name == "black_S"
        ...
        if (name == "black_NTSC_352x240" || name == "black_PAL_352x288")
            jpeg = "black";

        else
            if (name == "black_NTSC_352x480" || name == "black_PAL_352x576")
                jpeg = "black_S";

            else
                if (name == "black_NTSC_704x480" || name == "black_PAL_704x576")
                    jpeg = "black_M";

                else
                    if (name == "black_NTSC_720x480" || name == "black_PAL_720x576")
                        jpeg = "black_L";
...
//
//    Returns rough estimate of average gop_bytes length on success, fatal
//    on fail.
// ----------------------------------------------------------------------------
...
{
    std::string jpeg = jpegfile.getName();
                ...
                (binDir / "jpeg2yuv").string(),  // app1 name
    {
        ...
        (ntsc ? "29.97" : "25"),
            "-n",
            std::to_string (testFrames).c_str(),
            "-j",
            jpeg.c_str()
        },
    // Piped to:
    ...
    std::ifstream mini2File (m2vName, std::ios::binary | std::ios::ate);

    if (! mini2File.is_open())
        FATAL ("Can't find input file " + std::string (m2vName));
