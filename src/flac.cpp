...
void flacHeader::display (const FLAC__StreamMetadata *meta, const char* prefix, std::ostream &stream)
{
    stream << prefix <<
           "min_blocksize    :" << meta->data.stream_info.min_blocksize << std::endl << prefix <<
           "max_blocksize    :" << meta->data.stream_info.max_blocksize << std::endl << prefix <<
           "min_framesize    :" << meta->data.stream_info.min_framesize << std::endl << prefix <<
           "max_framesize    :" << meta->data.stream_info.max_framesize << std::endl << prefix <<
           "sample_rate      :" << meta->data.stream_info.sample_rate << std::endl << prefix <<
           "channels         :" << meta->data.stream_info.channels << std::endl << prefix <<
           "bits_per_sample  :" << meta->data.stream_info.bits_per_sample << std::endl << prefix <<
           "total_samples    :" << meta->data.stream_info.total_samples << std::endl << prefix <<
           "md5sum           :" << hexStr (meta->data.stream_info.md5sum, 16) << std::dec << std::endl;
}
...
void flacHeader::readStreamInfo (waveHeader::wav_extensible *wave, FLAC__StreamMetadata *meta)
{
...
int flacHeader::write (std::fstream &out, FLAC__StreamMetadata *meta)
{
