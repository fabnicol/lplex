...
typedef struct
{
    ...
    int16_t group;
    std::string name, extractTo, jpeg;
...
extern lplexJob job;
extern std::vector<lpcmFile> Lfiles;
extern std::vector<dvdJpeg> jpegs;
extern std::vector<std::string> dirs;
extern std::vector<std::string> menufiles;
extern std::vector<infoFile> infofiles;

extern unsigned char bigBlock[BIGBLOCKLEN];

extern std::string defaultOutPath;
...
int checkName (std::string &jobName, bool trim = false);
...
class lFileTraverser //: public wxDirTraverser
{
    public:
       ...
       std::vector<std::string> filenames;
        ...
        std::string setRoot (const char *rootPath, int fromParent);
        void processFiles();

        void Traverse (const std::string& path);

        virtual void OnFile (const std::string& filename);
        virtual int OnDir (const std::string& dirname);
        virtual void OnOpenError (const std::string& openerrorname);
...
class dvdauthorXml
{
    public:
        ...
        dvdauthorXml (const std::string& xmlName, int tv = NTSC, int ct = 1, bool styler = 0, bool disable = 0) :
        ...
        int write (xmlContext context, const std::string& str = "", int flag = 0);

        static std::string timestampFractional (uint32_t f, bool ntsc, float boost = 0);
};
...
class dvdUtil
{
    public:
        ...
        static std::string timestamp (uint32_t f, bool ntsc);
        static std::string time (uint32_t f, bool ntsc);
