...
//    character std::string instead of printing to the console.
// ----------------------------------------------------------------------------


std::string ifoPrint_audio_attributes (audio_attr_t *attr)
{
    std::string info;
    ...
    while (1)
        {
            ...
            if (verbose)
                INFO (_f ("Opening title %02d (%d audio stream%s)\n",
...
int lpcm_video_ts::open (const char * VIDEO_TS, bool fatal)
{
    ...
    if (res == nullptr)
        {
           std::cerr << "[ERR] Cannot open /dev/null" << std::endl;
            throw;
        }
    ...
    if (res == nullptr)
        {
           std::cerr << "[ERR] Cannot open console" << std::endl;
            throw;
        }
    ...
    if (label == "" || label == "VIDEO_TS")
        label = volumeLabel (VIDEO_TS);

    if (verbose)
        {
            SCRN (std::string (INFO_TAG) + std::string ("Opening dvd '"))
    ...
    if (writeIndex < numPGCcells)
        {
            ...
            ECHO ("\n");
            STAT (txt << lFile->fName.string().substr (lFile->root) << std::endl);

            if (state & lplexAuthored)
                {
                    INFO (lpcmEntity::audioInfo (lFile->writer));
                    ECHO (" (" << lFile->writer->ct.max << " audio_bytess)\n");
                }
            ...
            if (lpcmProcessor::soundCheck (lFile, 1) && job->params & md5)
                {
                    ...
                    POST (setw (10) << flacHeader::bytesUncompressed (&lFile->fmeta) << " :" <<
                          ...
                          lFile->trim.padding << " :" <<
                          " " << lFile->fName.stem() << std::endl << setw (0));
#endif
                }
        }
...
{
    std::string vob = _f ("VIDEO_TS/VTS_%02i_1.VOB", titleset);
...
{
    std::string iName, outPath;
    struct infoFile *iFile;
    size_t udfStart;
    std::fstream outFile, img;

    if (! infofiles->size())
        return;

    outPath = (path ? path : (const char*) job->extractPath.string());
    img.open (job->inPath.string(), std::ios::binary | std::ios::in);
    ...
    for (int i = 0; i < infofiles->size(); i++)
        {
            ...
            img.seekg (iFile->lb * DVD_VIDEO_LB_LEN, std::ios::beg);
            outFile.open (iName, std::ios::binary | std::ios::out);
