...
class flacHeader
{
    public:
        ...
        static void readStreamInfo (waveHeader::wav_extensible *wave, FLAC__StreamMetadata *meta);
        ...
        static int write (std::fstream &out, FLAC__StreamMetadata *meta);
        static int write (uint8_t *buf, FLAC__StreamMetadata *meta);

        static int audit (const char *filename, FLAC__StreamMetadata *fmeta);
        static void display (const FLAC__StreamMetadata *meta, const char* prefix = "", std::ostream &stream =std::cerr);

        friend std::ostream& operator << (std::ostream& stream, const FLAC__StreamMetadata& f)
        {
            display (&f, "", stream);
            return stream;
        }
