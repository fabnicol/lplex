...
//
//    Arguments:
//       <prepend>      - number of_bytess prepended to stream (must already
//                        be present at start of bigBuf).
//       <pad>          - whether to allow padding of final sample if
//                        necessary for dvd sample alignment.
//
//    Returns number of_bytess to be padded at end of stream.
// ----------------------------------------------------------------------------
...
//    lpcmReader::read :
// ----------------------------------------------------------------------------
//    Buffered reading of audio file in dvd_bytes-order.
//
//    Arguments:
//       <buf>       - pointer to start of receiving buffer
//       <len>       - number of_bytess to transfer
//
//    Returns number of_bytess actually transferred
// ----------------------------------------------------------------------------
...
//    lpcmReader::swap2dvd :
// ----------------------------------------------------------------------------
//    Swaps given lpcm data buffer from wave to dvd_bytes-order.
//
//    Arguments:
...
//
//    Returns 0 on success, number of remaining unswapped_bytess if <count>
...
uint16_t waveReader::reset (const std::string& filename, int alignUnit)
{
#if 0
    ...
    if (! waveFile.is_open())
        FATAL ("Can't find input file " + std::string (filename));
...
//
//    Returns number of_bytess processed.
// ----------------------------------------------------------------------------
...
uint16_t flacReader::reset (const std::string&  filename, int alignUnit)
{
    if (is_valid())
        finish();
