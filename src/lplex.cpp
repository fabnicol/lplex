...
int author (dvdLayout &layout)
{
    ...
    if (job.params & customized)
        SCRN ("\n\n")
        INFOv (std::string ("Creating ")
               + std::string ( ( (const char*[])
        { "LPCM", "M2V", "MPEG", "DVD", "ISO", "LGZ"
        }) [ job.prepare - 3 ])
    + std::string (" : [ ")
    + std::string ( (job.tv == NTSC ? "NTSC" : "PAL"))
    + std::string ( (job.params & md5 ? " md5aware" : ""))
    + std::string ( (job.params & info ? " infofiles" : ""))
    + std::string ( (job.params & dvdStyler ? " dvdStyler" : ""))
    + std::string ( (job.trimCt == 0 ? txt.c_str() : "")) + std::string (" ]\n"));
    layout.configure();
    ...
    while (layout.getNext())
        {
           ...
           std::cerr << "m2v" << " " << layout.nameNow << ".m2v" << std::endl;
            ...
            std::string o1 = layout.nameNow + std::string (".mpg");
            std::string o2 = layout.nameNow + std::string (".m2v");
            std::string o3 = layout.nameNow + std::string (".lpcm");
            std::string Larg = _f ("%d:%d:%d",
           ...
           std::cerr << std::endl << std::endl;
            ...
            if (job.params & cleanup)
                {
                    remove (std::string (layout.nameNow + ".m2v").c_str());
                    remove (std::string (layout.nameNow + ".lpcm").c_str());
                }
        }
...
int mkisofs (const fs::path &isoPath, const fs::path &dvdPath, const std::string& name)
{
    ...
    INFO ("Creating disc image \'" + isoPath.string() + std::string ("\'\n"));
    INFO ("Using volume id \'" + volumeID + std::string ("\'\n\n"));
    ...
    while ( (context = dvd.getCell()))
        {
            ...
            if (c == 0)
                ...
                if (! (writer->state & lpcmEntity::metered) || editing ||
                            ...
                            switch (context)
                                {
                                    ...
                                    case _reopen:
                                        STAT ("Continuing " << writer->fName.stem() << std::endl);
                                        break;
                                }
            ...
            if (processErr)
                ...
                if (! (writer->state & lpcmEntity::metered))
                    {
                        LOG (writer->ct.now << " audio_bytess\n");
                    }
        }
    ...
    for (i = 0; i < infofiles.size(); i++)
        {
            ...
            std::string comp =  i_fName.filename().string();
...
//
//    Returns actual number of_bytess written
// ----------------------------------------------------------------------------
...
uint16_t writeUserData (lpcmEntity *lFile, uint8_t *userData)
{
    ...
    std::string tag = _f ("<Lplex ver=\"%s\" path=\"%s\" id=\"%d\" shift=\"%d\" md5prev=\"%s\"/>",
                     VERSION,
                    ...
                    while (attr.get())
                        {
                            if (! stricmp (attr.name, "ver"))
                                ...
                                if (! stricmp (attr.name, "path"))
                                    ...
                                    if (! stricmp (attr.name, "id"))
                                        lFile->index = atoi (attr.val);

                                    else
                                        if (! stricmp (attr.name, "shift"))
                                            lFile->trim.offset = atol (attr.val);

                                        else
                                            if (! stricmp (attr.name, "md5prev"))
            ...
            while (addr > _EOF)
                {
                    ...
                    _SOF = _EOF;
                    vobFile.seekp (0, std::ios::end);
                    _EOF += vobFile.tellp();
                }

            addr -= _SOF;
            //go to relative file offset PCI::RECI & insert flac header
            vobFile.seekp (addr, std::ios::beg);
            ...
            if (echo)
                {
                    ...
                    if (_verbose)
                       std::cerr << TINT_BLIP (_f ("  %2d.%d%% done", perThousand / 10, perThousand % 10).c_str()) << std::flush;
...
int addMenus (const char * filename, int tv, bool force)
{
    ...
    //std::string menuSpec( fName.string() );
#if 0
    class menuTraverser : public wxDirTraverser
    {
        public:
            ...
            virtual wxDirTraverseResult OnFile (const std::string& fName)
            {
            ...
            virtual wxDirTraverseResult OnDir (const std::string& dirname)
            {
                if (state & found)
                    return wxDIR_IGNORE;
    ...
    for (auto& p : fs::directory_iterator (VIDEO_TS))
        {
            ...
            std::string fName = Right (f.string(), 12);
                    ...
                    if (map[titleset] != titleset)
                        {
                            ...
                            fName.replace (4, 2, std::to_string (map[titleset]));
                        }
