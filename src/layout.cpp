...
//    dvdUtil::timestamp :
// ----------------------------------------------------------------------------
//    Returns hh:mm:ss.ff frame-accurate timestamp std::string.
//
//    Arguments:
...
std::string dvdUtil::timestamp (uint32_t f, bool ntsc)
{
    uint32_t fph;
...
std::string dvdUtil::time (uint32_t f, bool ntsc)
{
    uint16_t min;
...
//    dvdUtil::sampleSeam :
// ----------------------------------------------------------------------------
//    Returns the size of the minimum coherent sample block in_bytess
// ----------------------------------------------------------------------------
...
//    dvdUtil::audioFrame :
// ----------------------------------------------------------------------------
//    Returns the size in_bytess of 1 dvd audio frame (bytes per sec/600)
// ----------------------------------------------------------------------------
...
int dvdUtil::audioFrame (int sample_rate, int channels, int bits_per_sample)
{
    return sample_rate *  channels * bits_per_sample /
           (8 /*bits per_bytes*/ * 600 /*dvd lpcm audio frames per sec*/);
}
...
//    dvdUtil::AVseam :
// ----------------------------------------------------------------------------
//    Returns the minimum packet size in_bytess that is fully aligned to both
    ...
    if (fail)
        {
            ...
            for (int i = 0; i < 3; i++)
                if (fail & (0x01 << i))
                    {
                        //  ERR( std::string(devices[i].path) + std::string(": not enough space on device ") + std::string(device( devices[i].id )))
                        ERR (" - " + std::string (sizeStr (devices[i].space)) + std::string (" free )\n"));
                    }
    ...
    for (uint i = 0; i < Lfiles->size(); ++i)
        {
            ...
            char p[100];
            std::stringstream x;
            x << std::setw (10) << audioBytes << " :" << std::setw (10) << lFile->trim.len << std::endl;
            x.getline (p, 100);
            ...
            //      LOG( lFile->details << " : " <<
            //             ( editing ? lFile->fName.string() : lFile->fName.filename() ) << std::endl );

            if (lFile->type & lpcmFile::titleEnd)
                {
                    LOG (_f ("[col 1] -trim units: s(AVseam)=%d d,e(audioFrame)=%d n(sampleSeam)=%d_bytess.\n",
...
int dvdLayout::getNext()
{
    ...
    out.open (nameNow + ".lpcm", job->now & appending ?
              (writeFile->type & lpcmFile::titleStart ?
               std::ios::binary : std::ios::binary | std::ios::app) :
               std::ios::binary);
...
// ----------------------------------------------------------------------------
//    Returns hh:mm:ss.frac rounded timestamp std::string, as used by dvdauthor.
//
//    Arguments:
//       <f>         - absolute frame offset
//       <ntsc>      - whether tv system is ntsc
...
std::string dvdauthorXml::timestampFractional (uint32_t f, bool ntsc, float boost)
{
    uint16_t hour, min;
    uint64_t ticks = f * (ntsc ? 3003 : 3600);
    double sec = (double) ticks / 90000;
...
//       <str>       - std::string to process depending on <context>.
//
//    Returns 0 on success, fatal on failure
// ----------------------------------------------------------------------------


int dvdauthorXml::write (xmlContext context, const std::string& str, int flag)
{
    ...
    switch (context)
        {
            case open:
                xml.open (str, std::ofstream::out | std::ofstream::trunc);

                if (! xml.is_open ())
                   std::cerr << "[ERR] Could not open xml" << std::endl;
